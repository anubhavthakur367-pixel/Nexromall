...
export default function Deposit() {
  ...
  const address = process.env.NEXT_PUBLIC_DEPOSIT_ADDRESS || 'Configure NEXT_PUBLIC_DEPOSIT_ADDRESS in Vercel'
  ...
  async function submit(e: FormEvent) {
    ...
    const { error } = await s.from('deposits').insert({ user_id: user.id, amount: Number(amount), asset: 'USDT', network: 'TRC20', address, tx_hash: txHash.trim(), status: 'pending' })
    ...
    <div className="mx-auto max-w-5xl"><button onClick={() => router.back()} className="text-sm text-slate-400">← Back</button>
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        <section className="rounded-3xl border border-white/10 bg-white/[.03] p-6 sm:p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-400">Send USDT on the selected network, then submit your transaction hash. Your deposit stays pending until an administrator verifies it.</p>
        ...
        <aside className="rounded-3xl border border-white/10 bg-white/[.03] p-6 sm:p-8"><h2 className="text-xl font-bold">Send USDT to</h2><p className="mt-2 text-sm text-slate-400">TRC20 (USDT)</p><div className="mt-5 rounded-2xl border border-white/10 bg-black/30 p-4"><p className="break-all text-sm text-slate-200">{address}</p></div><div className="mt-6 rounded-2xl border border-amber-400/10 bg-amber-400/5 p-4 text-sm leading-6 text-amber-100">1. Send the exact amount. 2. Paste the transaction hash. 3. Wait for admin verification. 4. Daily missions unlock only after an eligible deposit is approved.</div><a href="/dashboard" className="mt-5 block rounded-xl border border-white/10 px-4 py-3 text-center text-sm font-bold">Back to dashboard</a></aside>
      </div>
    </div>
  </main>
}
